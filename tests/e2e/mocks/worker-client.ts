  ...
  private async processJob(job: MockJob) {
    ...
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.executionDelayMs),
      );
      ...
      if (res.ok) {
        ...
        console.error(
          `[MockWorker] Failed to submit result for ${job.id}: ${res.statusText}`,
        );
      }
    } catch (err) {
...
function isMockJob(value: unknown): value is MockJob {
  return (
    typeof value === "object" &&
    value !== null &&
    "id" in value &&
...
function getJobs(value: unknown): MockJob[] {
  if (Array.isArray(value)) return value.filter(isMockJob);
  if (typeof value !== "object" || value === null) return [];

  if ("jobs" in value && Array.isArray(value.jobs))
    return value.jobs.filter(isMockJob);
  if ("data" in value && Array.isArray(value.data))
    return value.data.filter(isMockJob);
  return [];
}
