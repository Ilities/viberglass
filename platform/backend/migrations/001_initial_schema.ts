...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn("ticket_system", "varchar(50)", (col) =>
      col
        .notNull()
        .check(
          sql`ticket_system IN ('jira', 'linear', 'github', 'gitlab', 'azure', 'asana', 'trello', 'monday', 'clickup')`,
        ),
    )
  ...
  await db.schema
    ...
    .addColumn("severity", "varchar(20)", (col) =>
      col
        .notNull()
        .check(sql`severity IN ('low', 'medium', 'high', 'critical')`),
    )
    ...
    .addColumn("auto_fix_status", "varchar(20)", (col) =>
      col.check(
        sql`auto_fix_status IN ('pending', 'in_progress', 'completed', 'failed')`,
      ),
    )
  ...
  // Indexes
  await db.schema
    .createIndex("idx_bug_reports_project_id")
    .on("bug_reports")
    .column("project_id")
    .execute();
  await db.schema
    .createIndex("idx_bug_reports_timestamp")
    .on("bug_reports")
    .column("timestamp")
    .execute();
  await db.schema
    .createIndex("idx_bug_reports_ticket_id")
    .on("bug_reports")
    .column("ticket_id")
    .execute();
  await db.schema
    .createIndex("idx_bug_reports_auto_fix_status")
    .on("bug_reports")
    .column("auto_fix_status")
    .execute();
  await db.schema
    ...
    .execute();
  await db.schema
    .createIndex("idx_auto_fix_queue_status")
    .on("auto_fix_queue")
  ...
  await sql`CREATE TRIGGER update_projects_updated_at BEFORE UPDATE ON projects FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();`.execute(
    db,
  );
  await sql`CREATE TRIGGER update_bug_reports_updated_at BEFORE UPDATE ON bug_reports FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();`.execute(
    db,
  );
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql`DROP TRIGGER IF EXISTS update_bug_reports_updated_at ON bug_reports;`.execute(
    db,
  );
  await sql`DROP TRIGGER IF EXISTS update_projects_updated_at ON projects;`.execute(
    db,
  );
