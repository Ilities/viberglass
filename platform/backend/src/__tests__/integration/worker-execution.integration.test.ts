...
const mockJobStatusUpdates: Map<string, { status: string; progress?: Record<string, unknown>; errorMessage?: string }> = new Map();

jest.mock('../../services/JobService', () => {
  return {
    JobService: jest.fn().mockImplementation(() => ({
      submitJob: jest.fn().mockResolvedValue({ jobId: 'test-job-id', status: 'queued' }),
      updateJobStatus: jest.fn().mockImplementation((jobId: string, status: string, updates?: any) => {
...
describe('Worker Execution Integration Tests', () => {
  ...
  const generateJobId = (): string => `integration-test-job-${Date.now()}-${Math.random().toString(36).substring(7)}`;
  ...
  describe('End-to-end success flow', () => {
    ...
    it('should handle different worker types', async () => {
      ...
      const lambdaClanker = createMockClanker({
        deploymentStrategy: { id: 'lambda', name: 'lambda', description: 'Lambda', createdAt: '2024-01-01T00:00:00Z' },
      });
      const ecsClanker = createMockClanker({
        id: 'ecs-clanker',
        deploymentStrategy: { id: 'ecs', name: 'ecs', description: 'ECS', createdAt: '2024-01-01T00:00:00Z' },
        deploymentConfig: {
          clusterArn: 'arn:aws:ecs:us-east-1:123456789:cluster/test-cluster',
          taskDefinitionArn: 'arn:aws:ecs:us-east-1:123456789:task-definition/test-task',
          subnetIds: ['subnet-1'],
      ...
      const dockerClanker = createMockClanker({
        id: 'docker-clanker',
        deploymentStrategy: { id: 'docker', name: 'docker', description: 'Docker', createdAt: '2024-01-01T00:00:00Z' },
        deploymentConfig: {
      ...
      // Execute Lambda job
      const lambdaResult = await executionService.executeJob(lambdaJob, lambdaClanker);
      expect(lambdaResult.workerType).toBe('lambda');
      expect(mockJobStatusUpdates.get(lambdaJob.id)?.progress?.workerType).toBe('lambda');

      // Execute ECS job
      const ecsResult = await executionService.executeJob(ecsJob, ecsClanker);
      expect(ecsResult.workerType).toBe('ecs');
      expect(mockJobStatusUpdates.get(ecsJob.id)?.progress?.workerType).toBe('ecs');

      // Execute Docker job
      const dockerResult = await executionService.executeJob(dockerJob, dockerClanker);
      expect(dockerResult.workerType).toBe('docker');
      expect(mockJobStatusUpdates.get(dockerJob.id)?.progress?.workerType).toBe('docker');
    });
  });
  ...
  describe('Transient error retries - real execution flow', () => {
    ...
    it('should exhaust retries and fail after too many transient errors', async () => {
      ...
      expect(finalUpdates?.errorMessage).toMatch(/Worker invocation failed after 4 attempts/);
    });
  });
  ...
  describe('Permanent error handling', () => {
    ...
    it('should store error message for permanent failures', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain('Lambda function name or ARN not configured');

      // Verify error stored in status
      const finalUpdates = mockJobStatusUpdates.get(job.id);
      expect(finalUpdates?.errorMessage).toContain('Lambda function name or ARN not configured');
    });
  });
  ...
  describe('WorkerInvokerFactory integration', () => {
    ...
    it('should throw error for unknown worker type', () => {
      ...
      const invalidClanker = createMockClanker({
        deploymentStrategy: { id: 'unknown', name: 'unknown', description: 'Unknown', createdAt: '2024-01-01T00:00:00Z' },
      });
  ...
  describe('Test isolation', () => {
    it('should not interfere between tests with unique job IDs', async () => {
      ...
      setupMockInvokerPermanentFailure();
      await executionService.executeJob(job2, { ...clanker, deploymentConfig: {} });
