...
describe('API Endpoints', () => {
  ...
  describe('API Documentation', () => {
    it('should return API documentation', async () => {
      ...
      expect(response.body.title).toBe('ViBug Receiver API');
  ...
  describe('Webhooks API', () => {
    ...
    it('should validate trigger-autofix parameters', async () => {
      ...
      expect(response.body.error).toBe('ticketId and ticketSystem are required');
    });
...
export const createMockBugReportData = () => {
  return {
    ...
    metadata: {
      ...
      screen: { width: 1920, height: 1080, viewportWidth: 1920, viewportHeight: 1080, pixelRatio: 1 },
      network: { userAgent: 'test-agent', language: 'en-US', cookiesEnabled: true, onLine: true },
      console: [],
      errors: [],
