  ...
  return (
    <>
      ...
      <div className="mt-8 grid gap-8 sm:grid-cols-3">
        <Stat title="Total revenue" value={`$${event.totalRevenue.toLocaleString()}`} change={`${event.totalRevenueChange >= 0 ? '+' : ''}${event.totalRevenueChange}%`} />
        <Stat
          title="Tickets sold"
          value={`${event.ticketsSold}/${event.ticketsAvailable}`}
          change={`${event.ticketsSoldChange >= 0 ? '+' : ''}${event.ticketsSoldChange}%`}
        />
        <Stat title="Pageviews" value={event.pageViews.toLocaleString()} change={`${event.pageViewsChange >= 0 ? '+' : ''}${event.pageViewsChange}%`} />
      </div>
      ...
      <Table className="mt-4 [--gutter:--spacing(6)] lg:[--gutter:--spacing(10)]">
        ...
        <TableBody>
          {orders.map((order) => (
            <TableRow key={order.id} href={`/orders/${order.id}`} title={`Order #${order.id}`}>
