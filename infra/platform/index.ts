...
// Backend database-secret encryption key (SecureString in SSM, generated by Pulumi).
// Encrypts secrets stored in the database, e.g. the model key and repository
// token saved by setup. Rotating it makes existing database secrets unreadable.
const secretsEncryptionKey = new random.RandomPassword(
  `${config.environment}-viberglass-secrets-encryption-key`,
  {
    length: 64,
    special: false,
  },
...
const secretsEncryptionKeyParam = new aws.ssm.Parameter(
  `${config.environment}-viberglass-secrets-encryption-key`,
  {
    name: `/viberglass/${config.environment}/backend/secrets-encryption-key`,
    type: "SecureString",
    value: secretsEncryptionKey.result,
    keyId: kmsKeyId,
    overwrite: true,
    tags: config.tags,
  },
...
const backendEcs: BackendEcsOutputs = createBackendEcs({
  ...
  secretsEncryptionKeySsmArn: secretsEncryptionKeyParam.arn,
  slackSsm: {
