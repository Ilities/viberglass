...
| Gemini CLI is deprecated; Antigravity CLI (`agy`) replaces it | `packages/agents/agent-gemini`, Google provider binding | Announced 2026-05-19; since 2026-06-18 Gemini CLI answers consumer accounts with HTTP 410 but still serves paid Gemini API keys, so setup's Google binding works for now. To do (Jussi, 2026-09-24): add an `agent-antigravity` plugin (install `curl -fsSL https://antigravity.google/cli/install.sh \| bash`; official docs say it reads `GEMINI_API_KEY` when using an API key; check ACP support for live sessions), make it the `google` provider's default binding, and retire `agent-gemini` once it's covered. Adding it touches the platform's hardcoded agent lists (`packages/agents/README.md`). |
| Research and plan documents should be shown through a markdown renderer | ticket phase document panels | Future work (Jussi, 2026-09-24). Related to quick win #7 (bold inside list items). |
...
- **Default models** for OpenCode providers (and Moonshot) are pinned in the plugin bindings and will age; they're changeable per runner under Advanced. OpenCode Go defaults to the cheaper `opencode-go/deepseek-v4.1-flash` (Jussi, 2026-09-24), which its key check also names. Runners created before the change keep `kimi-k3` until setup runs again.

**Done (2026-09-24): the repository step.** `POST /api/setup/repository` `{ repository, token }` (admin only) reads `owner/repo`, a GitHub URL or an SSH address; one `GET /repos/{owner}/{repo}` checks read and push (`permissions.push`, plus the `repo`/`public_repo` scope for classic tokens) and returns GitHub's canonical name, URL and default branch. Plain-language failures: token rejected, not found (with the fine-grained "Repository access" hint), read-only, SSO/forbidden, rate limited, unreachable. It reuses the active GitHub connection (or creates "GitHub") and replaces the token on its default credential; a token credential that isn't default is reused and made default rather than duplicated; on a fresh workspace it creates a `GITHUB_TOKEN` secret. Model keys and repository tokens share `SetupSecretStore`. **Verified** live with an invalid token (401 → rejected) and with the dev database's classic token against `ilities/token.observer` (read + push, `main`) and a missing repo. **Limit:** a fine-grained token's "Pull requests" permission can't be read, so a token without it fails only when the first PR is opened. GitHub.com only; GitHub Enterprise, GitLab and Bitbucket stay under Advanced.

**Done (2026-09-24): the space step.** `POST /api/setup/space` `{ name, repository, baseBranch? }` (admin only) creates the project (Viberglass-native tasks, nothing else to choose), links the active GitHub connection and saves the SCM config with its default credential, so readiness's repository and credential checks pass. It needs the repository step first (409 otherwise). There's no transaction across the DAOs, so the step is idempotent instead: running it again with the same name finishes a half-created space, reuses one already on the same repository (case-insensitive), and refuses (409) a space on another repository or an archived one. Verified with unit tests and against the running dev backend: all three setup steps' failure paths return their plain-language errors, a member gets 403, nothing is written on failure, and neither keys nor tokens appear in the backend log. (The dev database's GitHub credential isn't marked default, so its space step answers "Connect the repository first" until the repository step runs once.)

**Done (2026-09-24): the default agent.** `POST /api/setup/agent` `{ provider }` (admin only) creates one runner, "Default agent" (slug `default-agent`), running the provider's default harness with the key's secret attached and the binding's `model`/`endpoint` in its agent config, and starts it at once (202; progress is the runner's status message on `GET /api/clankers/:id`). Compute: ECS managed when the ECS handler's own preflight passes (the stack's `VIBERATOR_ECS_*` settings), otherwise Docker in pre-built mode with the agent's catalog image. Running it again leaves an unchanged active or deploying agent alone, and reconfigures and restarts it otherwise (another provider, or a failed start). Needs the model-key step first (409).
- **Start logic** moved from the clankers route into `ClankerStartService`; the route and setup share it.
...
- **Seen, not fixed:** slugs drop dots (`token.observer` → `tokenobserver`) and the breadcrumb shows the slug, not the space name.
