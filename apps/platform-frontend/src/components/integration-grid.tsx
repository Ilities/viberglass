import { IntegrationCard, IntegrationCardSkeleton, type IntegrationCardData } from '@/components/integration-card'
import { TicketSystem } from '@viberglass/types'

interface IntegrationGridProps {
  integrations: IntegrationCardData[]
  hrefBase?: string
  isLoading?: boolean
  configured?: Partial<Record<TicketSystem, boolean>>
}
    ...
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {sortedIntegrations.map((integration) => (
        <IntegrationCard
          ...
          hrefBase={hrefBase}
          isConfigured={configured ? configured[integration.system] : undefined}
        />
      ))}
    </div>
  )
