...
- Phase 4.1: Allow Frontend to Invoke Workers (inserted after Phase 4 for urgent UI-to-backend integration)
- Phase 4.2: Testing (inserted after Phase 4.1 for pragmatic testing of worker execution flow)
- Phase 4.3: Application organization and structural refactoring (inserted after Phase 4.2 for code quality improvements)
- Phase 4.4: E2E Flow Verification (inserted after Phase 4.3 for happy path verification)
- Phase 11.1: Remove Unused Frontend Infrastructure (inserted after Phase 11 for cost cleanup)
- Phase 11.2: Amplify Frontend Infrastructure (inserted after Phase 11.1 for complete IaC)

**Key Decisions:**

- Provider interface pattern for cloud-agnostic credential storage (enables AWS, other clouds, local)
- Decimal phase numbering for urgent insertions (clear semantic for inserted work)
- SSM Parameter Store for secure credential access
- Pragmatic testing approach (focus on complex logic, skip trivial tests)
- Validation factory pattern to eliminate code duplication
- AWS Amplify SSR over S3+CloudFront static export (Next.js 15 dynamic routes)
- Environment-specific secrets via GitHub Environments + SSM
- OIDC authentication for all AWS access (no long-lived credentials)
- Two-tier secret architecture: deployment secrets (Phase 12) distinct from tenant credentials (Phase 1)

**Issues Resolved:**

- Tenant credential isolation via provider interface
- Worker result callback with retry logic
- Worker execution across Lambda/ECS/Docker with orphan detection
- Frontend-to-backend worker invocation flow
- Job status polling with toast notifications
- Clanker static and runtime status with health checks
- Webhook integration with signature validation and deduplication
- Local development environment with Docker compose
- Complete AWS infrastructure via Pulumi
- CI/CD pipeline with GitHub Actions and OIDC
- Amplify SSR deployment with Pulumi provisioning
- Centralized secret management for deployment targets

**Issues Deferred:**

- Real-time status via WebSocket/SSE (polling sufficient for v1.0)
- Multi-cloud credential providers (Azure Key Vault, GCP Secret Manager deferred to v2.0)

**Technical Debt Incurred:**

- None significant - refactoring phases (4.3, 11.1) proactively addressed debt
