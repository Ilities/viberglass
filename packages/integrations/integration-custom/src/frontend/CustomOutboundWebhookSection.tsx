...
type CustomOutboundWebhookSectionProps = SelfManagedOutboundWebhookSectionProps
...
export function CustomOutboundWebhookSection({
  ...
  useEffect(() => {
    ...
    async function loadDestinations() {
      ...
      setIsLoading(true)
      try {
        const configs = await onGetOutboundWebhooks(integrationEntityId)
        if (!isMounted) {
          return
        }
  ...
  const onSaveDestination = async () => {
    ...
    setIsSaving(true)
    try {
      const saved = await onSaveOutboundWebhook(
        integrationEntityId,
        {
          active: form.active,
          auth: authPayload,
  ...
  const onDeleteDestination = async () => {
    ...
    setIsSaving(true)
    try {
      await onDeleteOutboundWebhook(integrationEntityId, selectedDestination.id)
  ...
  const onTestDestination = async () => {
    ...
    try {
      const result = await onTestOutboundWebhook(integrationEntityId, destinationId, eventType)
      setLastTestResult({
        ...result,
