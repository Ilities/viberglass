...
import type { AutoFixDetectionConfig, CustomFieldMapping, PMIntegration } from './types'
...
export abstract class BasePMIntegration implements PMIntegration {
  ...
  abstract updateTicket(ticketId: string, updates: ExternalTicketUpdate): Promise<void>
  ...
  protected formatBugReportDescription(ticket: Ticket): string {
    ...
    if (ticket.metadata.console && ticket.metadata.console.length > 0) {
      ...
      const errorLogs = ticket.metadata.console.filter((log) => log.level === 'error')
      const warningLogs = ticket.metadata.console.filter((log) => log.level === 'warn')
  ...
  protected shouldEnableAutoFix(ticket: Ticket): boolean {
    ...
    return this.autoFixConfig.projectSettings.enableForSeverity.includes(ticket.severity)
  }
